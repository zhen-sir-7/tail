...
const CHECK_TEMPLATES = {
  'silent-failure': `// silent-failure: 静默错误 (提 PR)
    checks.push({
  ...
  'infinite-loading': `// infinite-loading: 缺少超时 (提 PR)
    checks.push({
  ...
  'missing-input-validation': `// missing-input-validation: 缺少输入验证 (提 PR)
    checks.push({
  ...
  'credential-exposure': `// credential-exposure: 凭证硬编码 (提 PR)
    checks.push({
  ...
  'data-corruption': `// data-corruption: 数据完整性 (提 PR)
    checks.push({
...
export function apply(learnedPath, targetEnginePath) {
  ...
  const injectEnd = '\n    // --- end data-driven ---';
  if (engine.includes('// --- data-driven checks (insights)')) {
    console.log('  ℹ engine.js 已包含 insights 注入的检查，替换旧代码');
    const start = engine.indexOf('// --- data-driven checks (insights)');
    const end = engine.indexOf('// --- end data-driven', start);
    if (end !== -1) {
      engine = engine.slice(0, start) + engine.slice(end + injectEnd.length);
    }
  }
  ...
  injectCode += '\n    // --- end data-driven ---\n';
  const newEngine = engine.slice(0, idx) + injectCode + engine.slice(idx);
